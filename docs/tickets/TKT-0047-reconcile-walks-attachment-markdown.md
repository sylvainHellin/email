...
One collision the shared predicate must account for: a server-side IMAP folder literally named "Attachments" used to be slugified into `<account>/attachments` (by `config::slugify_mailbox_name`, deleted with the rest of the path-shaped identity in #0064), the same path as the attachment mirror dir — a pre-existing on-disk collision, so sync would already interleave that folder's mail with mirrored attachments. Until the predicate can tell them apart, the accurate framing of the calendar loader's residual risk is "an IMAP folder named Attachments loses its invites", not "a hand-created custom mailbox".
